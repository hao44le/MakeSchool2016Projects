...
struct Deck {
    var cards:[Card]
    init(){
        let ranks : [Rank] = Rank.allValues()
        let suits : [Suit] = Suit.allValues()
        var cards : [Card] = []
        for rank in ranks {
            for suit in suits {
    ...
    func drawOne()->Card{
        let position = Int(arc4random_uniform(52))
        //print(position)
...
let deck = Deck()
let firstCard = deck.drawOne()
let secondCard = deck.drawOne()
if(firstCard.rank.rawValue > secondCard.rank.rawValue){
    print("The player won with the \(firstCard.rank) of \(firstCard.suit)!")
} else if(firstCard.rank.rawValue == secondCard.rank.rawValue){
    if(firstCard.suit.rawValue > secondCard.suit.rawValue){
        print("The player won with the \(firstCard.rank) of \(firstCard.suit)!")
    } else {
        print("The computer won with the \(secondCard.rank) of \(secondCard.suit)!")
    }
